...
.kpi-card.clickable:hover { border-color: rgba(14, 138, 138, .45); transform: translateY(-2px); }
.kpi-head { display: flex; align-items: baseline; justify-content: space-between; gap: 8px; }
.kpi-title { font-size: 12.5px; font-weight: 600; color: #4A7672; letter-spacing: .02em; }
.kpi-en { font-size: 9.5px; letter-spacing: .12em; color: #8AA6A2; text-transform: uppercase; white-space: nowrap; }
.kpi-num { display: flex; align-items: baseline; gap: 5px; margin-top: 6px; }
.kpi-num b {
  font-size: 26px; font-weight: 700; letter-spacing: -.02em; color: #12403E;
  font-variant-numeric: tabular-nums; font-family: var(--font-num);
}
.kpi-num i { font-style: normal; font-size: 11.5px; color: #5A7A76; }
.kpi-meta { margin-top: 6px; font-size: 11px; color: #4A7672; display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
.kpi-glow { position: absolute; right: -28px; top: -28px; width: 84px; height: 84px; border-radius: 50%; opacity: .16; pointer-events: none; }
.accent-teal .kpi-glow { background: radial-gradient(circle, #0E8A8A 0%, transparent 70%); }
.accent-cyan .kpi-glow { background: radial-gradient(circle, #2BBFAF 0%, transparent 70%); }
.accent-amber .kpi-glow { background: radial-gradient(circle, #C77F1A 0%, transparent 70%); }
.accent-red .kpi-glow { background: radial-gradient(circle, #DC4A3C 0%, transparent 70%); }
.accent-teal .kpi-num b { color: #0E8A8A; }
.accent-red .kpi-num b { color: #DC4A3C; }
.accent-amber .kpi-num b { color: #C77F1A; }
