...
function scatterSeries(list) {
  ...
  return [
    { type: 'scatter', coordinateSystem: 'geo', symbolSize: 5,
      itemStyle: { color: '#AD570E', opacity: 0.55, borderColor: 'rgba(7,13,26,.6)', borderWidth: 0.5 },
      data: t.lost, zlevel: 2 },
    { type: 'scatter', coordinateSystem: 'geo', symbolSize: 5,
      itemStyle: { color: '#5C7266', opacity: 0.7, borderColor: 'rgba(7,13,26,.6)', borderWidth: 0.5 },
      data: t.park, zlevel: 2 },
    { type: 'effectScatter', coordinateSystem: 'geo', symbolSize: 7,
      rippleEffect: { brushType: 'stroke', scale: 3.2 },
      itemStyle: { color: '#17A05E', shadowBlur: 8, shadowColor: 'rgba(23,160,94,.8)' },
      data: t.run, zlevel: 3 }
  ]
}
...
const mapBase = {
  ...
  label: { show: false },
  itemStyle: { borderColor: 'rgba(138,162,148,.35)', borderWidth: 0.8, areaColor: '#E3EFEA' },
  emphasis: { label: { show: true, fontSize: 12, color: '#1A2B22' }, itemStyle: { areaColor: '#BFE0D8' } },
...
function renderChina() {
  ...
  chart.setOption({
    ...
    visualMap: {
      ...
      text: ['车辆数', ''],
      textStyle: { color: '#5C7266', fontSize: 11 },
      inRange: { color: ['#E9F4EF', '#8CCFAA', '#17A05E'] }
    },
...
function renderChinaHeat() {
  ...
  chart.setOption({
    ...
    visualMap: {
      ...
      inRange: { color: ['rgba(21,94,117,.2)', 'rgba(23,160,94,.55)', 'rgba(52,245,197,.9)', '#E6FFFA'] }
    },
...
.mode-switch {
  ...
  background: rgba(255, 255, 255, .86); border: 1px solid rgba(23, 160, 94, .2);
  border-radius: 10px; padding: 4px; backdrop-filter: blur(8px);
}
.mode-switch button {
  font: inherit; font-size: 12px; color: #5C7266; background: none; border: 0;
  border-radius: 7px; padding: 6px 12px; cursor: pointer;
}
.mode-switch button.on { background: rgba(23, 160, 94, .16); color: #17A05E; font-weight: 600; }
.back-chip {
  position: absolute; left: 14px; top: 14px; z-index: 6; cursor: pointer;
  background: rgba(255, 255, 255, .86); border: 1px solid rgba(23, 160, 94, .2);
  color: #17A05E; font-size: 12px; font-weight: 500;
  border-radius: 999px; padding: 7px 16px; backdrop-filter: blur(8px);
}
.back-chip:hover { background: rgba(23, 160, 94, .14); }
.map-error {
