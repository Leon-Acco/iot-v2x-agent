...
public class SupervisorRouter {
    ...
    private static final String SYSTEM =
            ...
            + "- cross：仅当问题明确需要跨两个及以上数据域综合、且无法归入任何单一专家时才选它；"
            + "cross 是最慢的路径（全工具多轮推理），能归入单专家的绝不要给 cross。\n"
            + "注意：画图/可视化类请求不是 cross 的理由——每位专家都能生成图表，路由到数据所在域的专家即可。\n"
            + "- meta：关于平台自身的问题（数据源是什么/你能做什么/有哪些能力/你是谁），不调工具\n"
            + "- chat：问候、闲聊、与车辆数据无关的问题\n"
            + "任务固化类请求（存为任务/保存这个查询/定时自动查/每天巡检）：路由到与刚才查询相关的领域专家"
            + "（问题里带告警就 alarm、带里程就 mileage）；没有任何领域词时选 cross。"
            + "每位专家都有 save_task 工具，能把查询固化为手动/定时任务。\n"
